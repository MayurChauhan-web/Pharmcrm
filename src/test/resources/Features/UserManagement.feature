Feature: Full User Management Flow
 ...
 Scenario: Create user without Patient Module access and check restrictions
  ...
  Scenario: User with 'View' and 'Add' permission for Profile
  Given I log in as admin
  When I create a new user with email "testuser1_static@mailinator.com"
  And I create a profile with 'View' and 'Add' permission for Setup Module → Profile
  And I assign the profile and workspace to the user
  And I reset the user's password
  And I log in using the new user
  Then the user should be able to view and add profiles
  And the user should not be able to edit or delete any profiles


  Scenario: User with 'View' and 'Edit' permission for Profile
  Given I log in as admin
  When I create a new user with email "testuser1_static@mailinator.com"
  And I create a profile with 'View' and 'Edit' permission for Setup Module → Profile
  And I assign the profile and workspace to the user
  And I reset the user's password
  And I log in using the new user
  Then the user should be able to view and edit profiles
  And the user should not be able to add or delete any profiles

Scenario: User with 'View' and 'Add' permission but no 'Edit' or 'Delete' for Profile
  Given I log in as admin
  When I create a new user with email "testuser1_static@mailinator.com"
  And I create a profile with 'View' and 'Add' permission for Setup Module → Profile
  And I assign the profile and workspace to the user
